...
给yolov6 的onnx拼接几个操作
实现将yolov6 的output: [1, 8400,85]  # 85在我们这里是9

经切片和reshape操作 变为2个节点

boxes input : [batch_size, number_boxes, number_classes, number_box_parameters]
               [x_min, y_min, x_max, y_max]
               [8732, 100, 4]  8732个box, 100个类别， 4个坐标
scores input: [batch_size, number_boxes, number_classes]

# 对于上述问题：

boxes input: [1,8400, 1, 4]
scores_input: [1, 8400, 80] # 80是类别个数，我们是4个人类别


# 需要如何处理呢？

85这个维度的排列是：[x1_center,y_center,w,h, prob,80个类别的conf]  4, 1, 80  == [1,25200,4], [1,25200,1], [1,25200,80]

boxes: [1,8400,1,4]  # x_center,y_center, w,h--> x1,y1,x2,y2

socres: [1,8400,80]  # 80个类别的概率
...
graph = gs.import_onnx(onnx.load("./last_ckpt.onnx"))


# 添加计算类别概率的结点

# input
origin_output = [node for node in graph.nodes if node.name == "Concat_260"][0]
...
ends_conf = gs.Constant("ends_conf",values=np.array([1,8400,9],dtype=np.int64))
...
onnx.save(gs.export_onnx(graph),"./last_ckpt_1.onnx")
